...
function removeFirstOccurrences(str, value) {
  ...
  const lengthSubStr = value.length;
  if (indexSubStr < 0) {
    return str;
  }
  return str.slice(0, indexSubStr) + str.slice(indexSubStr + lengthSubStr);
}
...
function removeLastOccurrences(str, value) {
  const indexSubStr = str.lastIndexOf(value);
  const lengthSubStr = value.length;
  if (indexSubStr < 0) {
    return str;
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const substr = arr[i].split('').reverse().join('');
    resArr.push(substr);
  }
